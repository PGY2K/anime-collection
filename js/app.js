...
let addAnimeSearchTimer=null;
let addAnimeSearchRequest=0;
function clearAddAnimeSearchTimer(){if(addAnimeSearchTimer){clearTimeout(addAnimeSearchTimer);addAnimeSearchTimer=null}}
function searchResultDetailsUrl(anime){return `anime.html?anilist_id=${encodeURIComponent(anime.id)}`}
async function runAddAnimeSearch(){
  clearAddAnimeSearchTimer();
  const input=document.getElementById("addAnimeSearchInput"),root=document.getElementById("addAnimeResults"),search=input.value.trim(),requestId=++addAnimeSearchRequest;
  if(!search){root.innerHTML='<div class="empty-state">Search for an anime to begin.</div>';return}
  root.innerHTML='<div class="loading">Searching…</div>';
  try{
    const results=await searchAniListPage(search);
    if(requestId!==addAnimeSearchRequest||input.value.trim()!==search)return;
    root.innerHTML=results.length?results.map(a=>{const title=a.title?.english||a.title?.romaji||"Untitled",poster=a.coverImage?.large||a.coverImage?.medium||"",detailsUrl=searchResultDetailsUrl(a);return `<article class="search-result-card${matAdultPosterClass(a.isAdult)}"><a class="search-result-poster search-result-poster-link" href="${detailsUrl}" aria-label="Open ${escapeHtml(title)} details"><img src="${escapeHtml(poster)}" alt="${escapeHtml(title)} poster">${matAdultPosterOverlay(a.isAdult)}</a><div><h3 class="search-result-title">${escapeHtml(title)}</h3><div class="search-result-meta">${escapeHtml(a.format||"Anime")} • ${a.episodes?`${a.episodes} episodes`:"Episode count unavailable"} • ${a.seasonYear||"Year unavailable"}</div><small class="franchise-search-note">Select the poster to view the title or franchise page.</small></div><button class="select-anime-btn" type="button" data-id="${a.id}">Add</button></article>`}).join(""):'<div class="empty-state">No matching anime found.</div>';
    root.querySelectorAll(".select-anime-btn").forEach(btn=>btn.addEventListener("click",async()=>{const anime=results.find(x=>String(x.id)===btn.dataset.id);btn.disabled=true;btn.textContent="Adding…";try{await addAnime(anime);await refreshCollection();closeAddAnimeModal();input.value="";++addAnimeSearchRequest;root.innerHTML='<div class="empty-state">Search for an anime to begin.</div>'}catch(e){alert(e.message||"Could not add anime.");btn.disabled=false;btn.textContent="Add"}}));
  }catch(e){if(requestId!==addAnimeSearchRequest)return;root.innerHTML='<div class="error">Search failed. Please try again.</div>'}
}
function scheduleAddAnimeSearch(){clearAddAnimeSearchTimer();const input=document.getElementById("addAnimeSearchInput"),root=document.getElementById("addAnimeResults");if(!input.value.trim()){++addAnimeSearchRequest;root.innerHTML='<div class="empty-state">Search for an anime to begin.</div>';return}root.innerHTML='<div class="loading">Waiting for you to finish typing…</div>';addAnimeSearchTimer=setTimeout(runAddAnimeSearch,300)}
function initModal(){const modal=document.getElementById("addAnimeModal"),input=document.getElementById("addAnimeSearchInput");document.getElementById("openAddAnimeBtn").onclick=openAddAnimeModal;document.getElementById("closeAddAnimeBtn").onclick=closeAddAnimeModal;input.addEventListener("input",scheduleAddAnimeSearch);input.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();runAddAnimeSearch()}});modal.addEventListener("click",e=>{if(e.target===modal)closeAddAnimeModal()})}
