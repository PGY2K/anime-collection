...
async function detailsSyncActiveRecommendationRating(mediaId, rating) {
...
async function initializeRecommendationControl(record,title,media){
  ...
  const open=async()=>{
    ...
    const state=document.getElementById("recommendCurrentState"),message=document.getElementById("recommendMessage"),removeButton=document.getElementById("removeRecommendationBtn"),confirmButton=document.getElementById("confirmRecommendBtn");
    message.textContent="";
    message.className="recommend-message";
    ...
    try{
      ...
      const same=setRecommendedState(active);
      if(active){
        state.innerHTML=same?`You are currently recommending <strong>${detailsEscapeHtml(active.title||title)}</strong>. You can update the note or remove the recommendation.`:`You are currently recommending <strong>${detailsEscapeHtml(active.title||"another title")}</strong>. Recommending <strong>${detailsEscapeHtml(title)}</strong> will replace it.`;
        confirmButton.textContent=same?"Update Recommendation":"Replace Recommendation";
        removeButton.classList.toggle("hidden",!same);
        if(same)document.getElementById("recommendationNote").value=active.note||"";
      }else{
        state.textContent="You do not currently have an active recommendation.";
        confirmButton.textContent="Recommend Anime";
      }
    }catch(e){state.textContent="Your current recommendation could not be checked, but you can still continue."}
  };
  ...
  document.getElementById("confirmRecommendBtn").onclick=async()=>{
    const confirm=document.getElementById("confirmRecommendBtn"),message=document.getElementById("recommendMessage");
    ...
    try{
      ...
      if(error)throw error;
      message.textContent="Recommendation saved.";message.className="recommend-message is-success";setTimeout(()=>location.reload(),500);
    }catch(error){message.textContent=error.message||"Could not save recommendation.";confirm.disabled=!eligibility.eligible;confirm.textContent="Recommend Anime"}
  };
}
...
function initializeEditor(record,media){
  ...
  document.getElementById("rateAnimeForm").onsubmit=async(event)=>{
    ...
    try{
      ...
      await detailsSyncActiveRecommendationRating(media.id, changes.overall_rating);
