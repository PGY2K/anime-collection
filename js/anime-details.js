...
function renderDetails(record,media){
  ...
  const rating=record?detailsAverage(record.story,record.animation,record.enjoyment):null;
  ...
  root.innerHTML=`
      ...
      <div class="details-hero-content">
        ...
        <div>
          ...
          <div class="details-actions">
            ...
            ${owned?`<span class="details-rating">${rating===null?"Not rated yet":`⭐ ${rating.toFixed(1)}`}</span>`:""}
            ${owned&&record.favorite?'<span class="favorite-badge">♥ Favorite</span>':""}
            ${owned?'<button class="edit-anime-btn" id="editAnimeBtn" type="button">Edit Anime</button><button class="remove-anime-btn" id="removeAnimeBtn" type="button">Remove from Collection</button>':'<button class="edit-anime-btn" id="addToQueueBtn" type="button">+ Add to Queue</button>'}
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="details-grid">
      ...
      <aside class="details-panel">
        ${owned?`<h2>Your Tracking</h2><div class="info-list"><div class="info-row"><span class="info-label">Story</span><span class="info-value">${record.story??"—"}</span></div><div class="info-row"><span class="info-label">Animation</span><span class="info-value">${record.animation??"—"}</span></div><div class="info-row"><span class="info-label">Enjoyment</span><span class="info-value">${record.enjoyment??"—"}</span></div><div class="info-row"><span class="info-label">Last Season</span><span class="info-value">${detailsEscapeHtml(record.last_season||"—")}</span></div><div class="info-row"><span class="info-label">Started Watching</span><span class="info-value">${detailsEscapeHtml(record.started_watching||"—")}</span></div></div>`:""}
